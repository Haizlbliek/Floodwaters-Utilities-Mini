...
public class EffectOverrideRectRepresentation : RectRepresentation {
	public new EffectOverrideData Data => this.pObj.data as EffectOverrideData;
	public EffectOverride obj;

	public EffectOverrideRectRepresentation(DevUI owner, string IDstring, DevUINode parentNode, PlacedObject pObj, string name) : base(owner, IDstring, parentNode, pObj, name) {
	...
	public class EffectOverridePanel : Panel, IDevUISignals {
		public EffectOverrideData Data => (this.parentNode as EffectOverrideRectRepresentation).Data;

		public EffectOverridePanel(DevUI owner, string IDstring, DevUINode parentNode, Vector2 pos) : base(owner, IDstring, parentNode, pos, new Vector2(250f, 25f), "Effect Override Rect") {
			this.RefreshUI();
		}
		...
		public void RefreshUI() {
			...
			this.size.y = -15f;
			if (this.Data.gradient >= -0.001f) {
				this.subNodes.Add(new ModifySlider(this.owner, "Gradient", this, new Vector2(5f, this.panelStep), "Gradient Direction"));
			}
		...
		public string GradientButtonText() {
			return "Gradient: " + (this.Data.gradient > -0.001f ? "Directional" : "None");
		}
		...
		public void Signal(DevUISignalType type, DevUINode sender, string message) {
			if (sender.IDstring == "Toggle_Gradient") {
				if (this.Data.gradient >= -0.001f) {
					this.Data.gradient = -1f;
				} else {
					this.Data.gradient = 0f;
				}
		...
		public class ModifySlider : Slider {
			...
			public override void Refresh() {
				...
				EffectOverrideData data = (this.parentNode.parentNode as EffectOverrideRectRepresentation).Data;
				switch (this.IDstring) {
					case "Gradient":
						this.NumberText = Mathf.RoundToInt(data.gradient * 360f).ToString();
						this.RefreshNubPos(data.gradient);
						break;
			...
			public override void NubDragged(float nubPos) {
				...
				(this.parentNode.parentNode as EffectOverrideRectRepresentation).obj.Refresh();
			}
		}
		...
		public class PaletteController : IntegerControl {
			...
			public override void Increment(int change) {
				EffectOverrideData data = (base.parentNode.parentNode as EffectOverrideRectRepresentation).Data;
				switch (this.IDstring) {
					case "Effect_Color_A":
						data.colorA += change;
						if (data.colorA < -1) {
